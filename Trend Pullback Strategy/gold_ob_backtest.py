"""
╔══════════════════════════════════════════════════════════════╗
║   GOLD OB — Order Block Backtester                          ║
...
STRATEGY LOGIC:
  ...
  3. Order Block = last opposite-color candle before the impulse
  ...
  5. SL below/above OB candle wick | TP at 3:1 R:R
...
5. Run: python3 gold_ob_backtest.py
"""
...
CONFIG = {
    ...
    # ── Structure Detection ────────────────────────────────────
    "SWING_LOOKBACK"      : 5,             # bars each side for swing high/low
    "OB_MAX_AGE"          : 80,            # bars before OB expires (unvisited)
    "OB_LOOKBACK"         : 25,            # bars back to find OB candle from swing

    # ── Strategy — Entry ──────────────────────────────────────
    "RR_RATIO"            : 3.0,           # 1:3 R:R (OB strategy targets high RR)
    "ATR_LEN"             : 14,
    "SL_BUFFER_MULT"      : 0.15,          # ATR fraction added below/above OB wick for SL
    "HTF_EMA"             : 50,            # 1H EMA used as higher-timeframe bias proxy
    "MIN_OB_BODY_MULT"    : 0.2,           # OB candle body must be > ATR * this

    # ── Strategy — Session (GMT) ───────────────────────────────
    "SESSION_START"       : 7,
    "SESSION_END"         : 18,            # London + NY combined
...
def add_indicators(df, cfg):
    ...
    # HTF bias via EMA (proxy for higher-timeframe trend)
    ...
    # Candle properties
    df['body']      = (c - o).abs()
    df['bull_bar']  = c > o           # up-close candle
    df['bear_bar']  = c < o           # down-close candle
    ...
    # Swing highs/lows (bar i is a swing high if it's the highest in [i-n .. i+n])
    # They become "known" to the loop n bars after they form (no lookahead)
...
def find_ob_candle(df, from_idx, lookback, ob_type, min_body):
    ...
    Returns a dict describing the OB zone, or None.
    """
    ...
    for j in range(from_idx, stop, -1):
        if df[col].iloc[j] and df['body'].iloc[j] >= min_body:
            ...
            return {
                'ob_high'    : max(ob_open, ob_close),   # body top
                'ob_low'     : min(ob_open, ob_close),   # body bottom
                'wick_high'  : df['high'].iloc[j],       # full candle high (for SL on bear OB)
                'wick_low'   : df['low'].iloc[j],        # full candle low  (for SL on bull OB)
                'formed_idx' : j,
...
def run_backtest(df, cfg):
    ...
    n            = cfg['SWING_LOOKBACK']
    start_bar    = n * 2 + 10   # enough history to confirm swings

    # Live structure tracking
    last_sh_price = None    # most recent confirmed swing high price
    last_sh_idx   = -1
    last_sl_price = None    # most recent confirmed swing low price
    last_sl_idx   = -1

    # BOS tracking (avoid re-creating OBs for the same BOS level)
    last_bull_bos = None
    last_bear_bos = None
    ...
    for i in range(start_bar, len(df)):
        ...
        # ── Update confirmed swings (confirmed n bars ago) ────
        conf_i = i - n
        if df['is_swing_high'].iloc[conf_i]:
            ...
            if last_sh_price is None or new_sh != last_sh_price:
                ...
                last_sh_idx   = conf_i
                last_bull_bos = None   # new swing = reset BOS trigger
        ...
        if (last_sh_price is not None
            ...
            # Bullish BOS → look for last bearish candle before the swing high
            ob = find_ob_candle(df, last_sh_idx, cfg['OB_LOOKBACK'], 'bear', min_body)
            if ob is not None:
                ob['dir']        = 'bull'
                ob['age']        = 0
                ob['mitigated']  = False
                ob['bos_price']  = last_sh_price
        ...
        if (last_sl_price is not None
            ...
            # Bearish BOS → look for last bullish candle before the swing low
            ob = find_ob_candle(df, last_sl_idx, cfg['OB_LOOKBACK'], 'bull', min_body)
            if ob is not None:
                ob['dir']        = 'bear'
                ob['age']        = 0
                ob['mitigated']  = False
                ob['bos_price']  = last_sl_price
                active_obs.append(ob)
        ...
        for ob in active_obs:
            ob['age'] += 1
            # Mitigated: price closes fully through the OB body
        ...
        if position is None and trades_today < cfg['MAX_TRADES_DAY'] and row['in_session']:
            for ob in active_obs:
                ...
                if ob['dir'] == 'bull' and row['bull_bias']:
                    # Price must retrace into the OB body from above
                    # and close at or above the OB body bottom (showing rejection)
                    if row['low'] <= ob['ob_high'] and row['close'] >= ob['ob_low']:
                        entry     = ob['ob_high']
                        sl_buffer = row['atr'] * cfg['SL_BUFFER_MULT']
                        sl        = ob['wick_low'] - sl_buffer
                        direction = 'long'

                elif ob['dir'] == 'bear' and row['bear_bias']:
                    # Price retraces up into OB body and closes at or below body top
                    if row['high'] >= ob['ob_low'] and row['close'] <= ob['ob_high']:
                        entry     = ob['ob_low']
                        sl_buffer = row['atr'] * cfg['SL_BUFFER_MULT']
                        sl        = ob['wick_high'] + sl_buffer
                        direction = 'short'

                if entry is not None and sl is not None:
                    sl_dist = abs(entry - sl)
                    if sl_dist < 0.5:          # too tight — skip
                        continue
                    ...
                    ob['mitigated'] = True     # one entry per OB
                    trades_today   += 1
                    break
...
# ═══════════════════════════════════════════════════════════════
#  PERFORMANCE METRICS  (identical to V3)
# ═══════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════
#  FTMO 2026 MONTE CARLO SIMULATOR  (identical to V3)
# ═══════════════════════════════════════════════════════════════
...
def plot_results(tdf, equity_s, metrics, ftmo, cfg):
    ...
    ax1.set_title('Equity Curve — GOLD OB | Real XAUUSD (Jan 2024 – Mar 2026)',
    ...
    if not tdf.empty:
        ...
        if len(wins_p) > 0:
            ax3.hist(wins_p, bins=20, color=GREEN, alpha=0.7,
                     label=f'Wins ({len(wins_p)})', density=True)
        if len(loss_p) > 0:
            ax3.hist(loss_p, bins=20, color=RED, alpha=0.7,
    ...
    fig.suptitle('GOLD OB — Order Block Strategy | FTMO 2026 Rules | Real XAUUSD Backtest',
                 color=GOLD, fontsize=15, fontweight='bold', y=0.99)
    plt.savefig('gold_ob_report.png', dpi=150, bbox_inches='tight', facecolor=BG)
    print("  Chart saved → gold_ob_report.png")
    plt.close()
...
def print_report(metrics, ftmo, cfg):
    ...
    print(f"\n{BOLD}{'═'*58}{W}")
    print(f"{BOLD}{Y}  GOLD OB — ORDER BLOCK BACKTEST REPORT{W}")
    print(f"{BOLD}{'═'*58}{W}")
    ...
    print(f"  Win Rate:            {c2(wr,40,35)}{wr:.1f}%{W}  (OB strategy: 35-45% typical)")
    ...
    checks = [
        ...
        ("Win Rate > 35%",           wr >= 35),   # OB has lower WR, higher RR
        ("Max DD < 8%",              dd < 8),
...
def main():
    print("\n╔══════════════════════════════════════════════════════╗")
    print("║   GOLD OB — Order Block Backtester + FTMO 2026      ║")
    print("║   Strategy: Structure + Order Block (Smart Money)   ║")
    print("╚══════════════════════════════════════════════════════╝\n")
    ...
    print(f"  {sh_count} swing highs | {sl_count} swing lows detected")

    print("\n► Step 3: Running backtest (OB entries)...")
    tdf, equity_s = run_backtest(df, CONFIG)
    if tdf.empty:
        print("  No trades executed. Try loosening filters (SWING_LOOKBACK, OB_MAX_AGE).")
        return
    longs  = (tdf['direction'] == 'long').sum()
    shorts = (tdf['direction'] == 'short').sum()
    print(f"  {len(tdf)} trades executed ({longs} longs, {shorts} shorts)")
    ...
    tdf.to_csv('gold_ob_trades.csv', index=False)
    print("  Trade log → gold_ob_trades.csv\n")
